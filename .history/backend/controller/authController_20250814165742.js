...
// ✅ Admin login (first check .env credentials, else fallback to DB)
export const adminLogin = async (req, res) => {
  try {
    const { email, password } = req.body;

    // 🔹 Step 1: Check hardcoded .env credentials
    if (email === process.env.ADMIN_EMAIL && password === process.env.ADMIN_PASSWORD) {
      const token = genToken(userId, "admin", email);
      res.cookie("token", token, {
        httpOnly: true,
        secure: false,
        sameSite: "None",
      ...
      return res.status(200).json({
        token,
        admin: { id: null, name: "Admin", email, role: "admin" },
      });
    }

    // 🔹 Step 2: Check in MongoDB (if admin user exists in DB)
